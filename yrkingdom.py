...
def computeTurn():
    ...
    if validations.validate_submitted_population(fw, dw, mi) == False:
        messagebox.showerror("Population Mismatch",
                             f"Your selections ({fw + dw + mi}) exceed your population - please try again")
        return

    # Validate Planted
    if pl == 0 and shared.turns[-1].Season == "growing":
        messagebox.showerror("Nothing Planted",
                             "You have planted no food!\n\nPlease enter the amount of food to plant.")
...
def computeReport():
    ...
    shared.turns.append(gameobjects.GameState(f=shared.turns[-1].Food, g=nextPlanted, p=shared.turns[-1].Population,
                                              fw=0, dw=0, m=0, s=shared.turns[-1].nextSeason(),
                                              j=shared.turns[-1].ElapsedSeasons + 1, fd=None, ad=None))
...
def showSplashWindow():
    ...
    splash_start_button = tk.Button(splashWindow, text="Start", font=button_font,
                                    command=lambda: splashStart(splashWindow))
...
def showInstructions():
    ...
    messagebox.showinfo("Instructions",
                        "The year consists of three long seasons, Winter, Growing and Harvest. Rice is planted every Growing Season. You must decide how much is planted.\n\n"
...
def showRitual():
    result = messagebox.askyesno("Village Ritual",
                                 f"We have survived for {shared.turns[-1].year()} years under your glorious control. "
...
def showReport(villages):
    ...
    messagebox.showwarning("Village Leader's Report",
                           f"In the {shared.turns[-1].Season} Season of year {shared.turns[-1].year()} of your reign, the kingdom has suffered these losses:\n\n"
                           + f"{populationMsg}\n\n"
                           + "FLOOD DAMAGE\n"
...
shared.turns = []
startGame = False
discord_id = '1296825858723676211'
...
start = int(time.time())
try:
    rpc_conn = pypresence.Presence(discord_id)
    rpc_conn.connect()
    rpc_conn.update(start=start)
except Exception:
    rpc_conn = None
